...
def 解析源代码新(
    ...
    for index, fp in enumerate(file_manifest):
        ...
        i_say = (
            prefix
            + f"请对下面的程序文件做一个概述文件名是{os.path.relpath(fp, project_folder)}，文件代码是 ```{file_content}```"
        )
        i_say_show_user = (
            prefix
    ...
    while True:
        ...
        this_iteration_gpt_response_collection = gpt_response_collection[
            : batchsize * 2
        ]
        file_rel_path = [
            os.path.relpath(fp, project_folder)
            for index, fp in enumerate(this_iteration_file_manifest)
        ]
        ...
        # 裁剪input
        inputs, this_iteration_history_feed = input_clipping(
            inputs=i_say, history=this_iteration_history, max_token_limit=2560
        )
        ...
        diagram_code = make_diagram(
            this_iteration_files, result, this_iteration_history_feed
        )
...
def make_diagram(this_iteration_files, result, this_iteration_history_feed):
    ...
    return build_file_tree_mermaid_diagram(
        ...
        this_iteration_history_feed[1::2],
        "项目示意图",
    )
...
def 解析项目本身(
    ...
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何python文件: {txt}"
        )
...
def 解析一个C项目的头文件(
    ...
    if os.path.exists(txt):
        ...
        return
    file_manifest = [
        f for f in glob.glob(f"{project_folder}/**/*.h", recursive=True)
    ] + [
    ...
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何.h头文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from 解析源代码新(
...
def 解析一个Java项目(
    ...
    if os.path.exists(txt):
        ...
        return
    file_manifest = (
        [f for f in glob.glob(f"{project_folder}/**/*.java", recursive=True)]
    ...
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何java文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from 解析源代码新(
...
def 解析一个前端项目(
    ...
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何前端相关文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from 解析源代码新(
...
def 解析一个Golang项目(
    ...
    if os.path.exists(txt):
        ...
        return
    file_manifest = (
        [f for f in glob.glob(f"{project_folder}/**/*.go", recursive=True)]
    ...
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何golang文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from 解析源代码新(
...
def 解析一个Rust项目(
    ...
    if os.path.exists(txt):
        ...
        return
    file_manifest = (
        [f for f in glob.glob(f"{project_folder}/**/*.rs", recursive=True)]
...
def 解析一个Lua项目(
    ...
    if os.path.exists(txt):
        ...
        return
    file_manifest = (
        [f for f in glob.glob(f"{project_folder}/**/*.lua", recursive=True)]
        + [f for f in glob.glob(f"{project_folder}/**/*.xml", recursive=True)]
    ...
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何lua文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from 解析源代码新(
...
def 解析一个CSharp项目(
    ...
    if os.path.exists(txt):
        ...
        return
    file_manifest = [
        f for f in glob.glob(f"{project_folder}/**/*.cs", recursive=True)
    ] + [f for f in glob.glob(f"{project_folder}/**/*.csproj", recursive=True)]
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何CSharp文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from 解析源代码新(
...
def 解析任意code项目(
    ...
    pattern_except_name = [
        ...
        if (
            _ != "" and _.strip().startswith("^") and not _.strip().startswith("^*.")
        )  # ^开始，但不是^*.开始
    ]
    # 生成正则表达式
    pattern_except = r"/[^/]+\.(" + "|".join(pattern_except_suffix) + ")$"
    pattern_except += (
        "|/(" + "|".join(pattern_except_name) + ")$"
        if pattern_except_name != []
        else ""
    )
    ...
    file_manifest = [
        ...
        for f in glob.glob(f"{extract_folder_path}/**/{pattern}", recursive=True)
    ...
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from 解析源代码新(
