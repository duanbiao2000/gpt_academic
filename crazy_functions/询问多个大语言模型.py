...
def 同时问询(
    ...
    llm_kwargs["llm_model"] = (
        MULTI_QUERY_LLM_MODELS  # 支持任意数量的llm接口，用&符号分隔
    )
...
def 同时问询_指定模型(
    ...
    # llm_kwargs['llm_model'] = 'chatglm&gpt-3.5-turbo&api2d-gpt-3.5-turbo' # 支持任意数量的llm接口，用&符号分隔
    llm_kwargs["llm_model"] = plugin_kwargs.get(
        "advanced_arg", "chatglm&gpt-3.5-turbo"
    )  # 'chatglm&gpt-3.5-turbo' # 支持任意数量的llm接口，用&符号分隔
