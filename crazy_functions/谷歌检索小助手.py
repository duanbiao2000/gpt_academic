...
def get_meta_information(url, chatbot, history):
    ...
    def string_similar(s1, s2):
        # 使用difflib库中的SequenceMatcher类，计算两个字符串的相似度
    ...
    if ENABLE_ALL_VERSION_SEARCH:
        ...
        def search_all_version(url):
            ...
            for result in soup.select(".gs_ri"):
                try:
                    # 从result中选取class为gs_rt的元素，并获取其a标签的href属性值，赋值给url变量
    ...
    for result in soup.select(".gs_ri"):
        ...
        try:
            citation = result.select_one(
                ".gs_fl > a[href*='cites']"
            ).text  # 引用次数是链接中的文本，直接取出来
        except Exception:
            citation = "cited by 0"
        abstract = result.select_one(
            ".gs_rs"
        ).text.strip()  # 摘要在 .gs_rs 中的文本，需要清除首尾空格
        ...
        if not is_match and ENABLE_ALL_VERSION_SEARCH:
            other_versions_page_url = [
                tag["href"]
                for tag in result.select_one(".gs_flb").select(".gs_nph")
                if "cluster" in tag["href"]
            ]
            if other_versions_page_url:
                ...
                is_match = (
                    paper is not None and string_similar(title, paper.title) > 0.90
                )
...
def 谷歌检索小助手(
    ...
    if len(meta_paper_info_list) == 0:
        yield from update_ui_lastest_msg(
            lastmsg="获取文献失败，可能触发了google反爬虫机制。",
            chatbot=chatbot,
            history=history,
            delay=0,
        )
