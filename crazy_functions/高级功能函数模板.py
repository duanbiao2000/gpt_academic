...
def 高阶功能模板函数(
    ...
    chatbot.append(
        (
            "您正在调用插件：历史上的今天",
            f"[Local Message] 请注意，您正在调用一个[函数插件]的模板，该函数面向希望实现更多有趣功能的开发者，它可以作为创建新功能函数的模板（该函数只有20多行代码）。此外我们也提供可同步处理大量文件的多线程Demo供您参考。您若希望分享新的功能模组，请不吝PR！{高阶功能模板函数示意图}",
        )
    )
...
def 测试图表渲染(
    ...
    chatbot.append(
        (
            "这是什么功能？",
            "一个测试mermaid绘制图表的功能，您可以在输入框中输入一些关键词，然后使用mermaid+llm绘制图表。",
        )
    )
    yield from update_ui(
        chatbot=chatbot, history=history
