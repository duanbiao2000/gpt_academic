...
def ipynb解释(
    ...
    inputs_array = [
        r"This is a Jupyter Notebook file, tell me about Each Block in Chinese. Focus Just On Code."
...
def 解析ipynb文件(
    ...
    chatbot.append(["函数插件功能？", "对IPynb文件进行解析。Contributor: codycjy."])
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
    ...
    if txt.endswith(".ipynb"):
        ...
        file_manifest = list(glob.glob(f"{project_folder}/**/*.ipynb", recursive=True))
    if not file_manifest:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何.ipynb文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from ipynb解释(
