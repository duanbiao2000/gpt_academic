...
class VideoResource(BaseModel):
    thought: str = Field(
        description="analysis of the search results based on the user's query"
    )
    title: str = Field(description="title of the video")
    author: str = Field(description="author/uploader of the video")
    bvid: str = Field(description="unique ID of the video")
    another_failsafe_bvid: str = Field(
        description="provide another bvid, the other one is not working"
    )
...
def get_video_resource(search_keyword):
    ...
    return search_videos(search_keyword)
...
def download_video(bvid, user_name, chatbot, history):
    ...
    # download audio
    chatbot.append((None, "下载音频, 请稍等..."))
    yield from update_ui(chatbot=chatbot, history=history)
    downloaded_files = yield from download_video(
        bvid, only_audio=True, user_name=user_name, chatbot=chatbot, history=history
    )
    ...
    yield from update_ui_lastest_msg(
        f"已完成的文件: <br/>{file_links}",
        chatbot=chatbot,
        history=history,
        delay=0,
    )
    chatbot.append((None, "即将下载视频。"))
    ...
    # download video
    chatbot.append((None, "下载视频, 请稍等..."))
    yield from update_ui(chatbot=chatbot, history=history)
    downloaded_files_part2 = yield from download_video(
        bvid, only_audio=False, user_name=user_name, chatbot=chatbot, history=history
    )
...
class Strategy(BaseModel):
    thought: str = Field(
        description="analysis of the user's wish, for example, can you recall the name of the resource?"
    )
    which_methods: str = Field(
        description="Which method to use to find the necessary information? choose from 'method_1' and 'method_2'."
    )
    method_1_search_keywords: str = Field(
        description="Generate keywords to search the internet if you choose method 1, otherwise empty."
    )
    method_2_generate_keywords: str = Field(
        description="Generate keywords for video download engine if you choose method 2, otherwise empty."
    )
...
def 多媒体任务(
    ...
    if "跳过联网搜索" not in user_wish:
        ...
        yield from update_ui_lastest_msg(
            lastmsg=f"发起互联网检索: {internet_search_keyword} ...",
            chatbot=chatbot,
            history=[],
            delay=1,
        )
    ...
    def run_gpt_fn(inputs, sys_prompt):
    ...
    # 关键词展示
    chatbot.append(
        (None, f"检索关键词已确认: {video_engine_keywords}。筛选中, 请稍等...")
    )
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面

    # 获取候选资源
    candadate_dictionary: dict = get_video_resource(video_engine_keywords)
    candadate_dictionary_as_str = json.dumps(
        candadate_dictionary, ensure_ascii=False, indent=4
    )
    ...
    analyze_res = run_gpt_fn(inputs, "")
    logger.info(analyze_res)
    video_resource: VideoResource = gpt_json_io.generate_output_auto_repair(
    ...
    chatbot.append((None, "下载中, 请稍等..."))
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面

    if video_resource and video_resource.bvid:
        logger.info(video_resource)
        downloaded = yield from download_video(
            video_resource.bvid, chatbot.get_user(), chatbot, history
        )
        if not downloaded:
            chatbot.append((None, "下载失败, 尝试备选 ..."))
            yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
            downloaded = yield from download_video(
                video_resource.another_failsafe_bvid,
                chatbot.get_user(),
                chatbot,
                history,
            )
...
@CatchException
def debug(
    bvid, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request
):
