...
def 知识库文件注入(
    ...
    try:
        ...
        """知识库接口：根据函数名推测，knowledge_archive_interface 可能用于与一个知识库或数据存档进行交互，比如存储、检索或更新知识库中的数据。"""
    ...
    if len(file_manifest) == 0:
        chatbot.append(
            [
                "没有找到任何可读取文件",
                "当前支持的格式包括: txt, md, docx, pptx, pdf, json等",
            ]
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    ...
    chatbot.append(
        [
            ...
            "正在预热文本向量化模组, 如果是第一次运行, 将消耗较长时间下载中文向量化模型...",
        ]
    )
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
...
@CatchException
def 读取知识库作答(
    txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request=-1
):
    # resolve deps
    try:
        # from zh_langchain import construct_vector_store
