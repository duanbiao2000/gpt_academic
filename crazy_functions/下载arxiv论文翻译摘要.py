...
def download_arxiv_(url_pdf):
    ...
    x = (
        ...
        .replace(
            "  ", " "
        )  # 这里进行了两次替换，可能是为了确保所有连续的空格都被替换为单个空格，即使第一次替换后仍然存在连续的空格。
    )
    return file_path, other_info


def get_name(_url_):  # sourcery skip: use-getitem-for-re-match-groups
...
def 下载arxiv论文并翻译摘要(
    ...
    try:
        ...
        report_exception(chatbot, history, a=f"解析项目: {txt}", b=f"下载pdf文件未成功")
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    ...
    i_say_show_user = (
        f"请你阅读以下学术论文相关的材料，提取摘要，翻译为中文。论文：{pdf_path}"
    )
